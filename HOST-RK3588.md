...
**结论：`DEFAULT_MAX_CONTEXT` 保持 288。** 解禁条件写在
`src/eidolon_models_tts/config.py` 的注释里，并由
`tests/test_tts_engine_command.py` 钉住（那条断言是留着被**故意**改掉的：修好
serve 循环的 cache 复位之后，连同它一起改）。
