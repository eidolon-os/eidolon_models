...
| TTS CosyVoice2 | NPU (RKLLM + RKNN) | 首包 **1.9 s**；稳态 rtf 分布**跨越 1.0**（中位 ~1.01，0.976–1.143）；干净上限 **一轮 ≤ ~7 s 音频** ✗ | §2.11 §2.12 §2.20 |
...
| CosyVoice2 | 绑 **A76 4–6** | 与 4–7 实测不可区分（§2.20 [E]）；选三核是为把 cpu7 留给 bge 与控制面 |
...
1. **NPU 无法分区。** `rkllm.h` 没有任何 NPU core 接口，两个 RKLLM 各抓满 3 核。
   **LLM 与 TTS 必须串行**，端到端按串行计算（§2.15）。
...
* **端到端首音 ≈ 4.06 s**（§2.17 末），其中 TTS 首包 1998 ms 占 49%、
  LLM 生成首句 1604 ms 占 40%。
* **CosyVoice2 稳态 rtf 分布跨越 1.0，且没有配置能把它压到 1 以下**（§2.18 §2.20）。
  满频下中位 ~1.01、区间 0.976–1.143，可干净播放的上限是**一轮回复 ≤ ~7 秒音频（26–39 字）**；50 字起
  必然出现听得见的断音。**分句不能规避，实测更糟**（实效 rtf 1.397 vs 1.127），
  因为每句要付 ~1.9 s 首包，而首包本身就是 NPU 忙时间，没有空闲可供重叠。
  **这是模型在这块板上的能力边界，不是争用造成的。**
