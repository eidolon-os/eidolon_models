...
* **段间流水（`preemptive_tts`）—— 仍值得做，但优先级下降。** 它解决的是段间静默，属于
  **首音延迟**问题，不是断音问题。
* **把 60 这个数固化成显式契约 —— 反而变成最重要的一项，已做。** 它此前是唯一挡住 126 字
  那条 −372 ms 边界的东西，却只是另一个仓库里的一个默认值，两边都没有写下"超过它会断音"。
  现在服务侧宣告它：`config.SAFE_TEXT_CHARACTERS = 60`（带上面那张表作为依据），并通过
  `/v1/info` 的 `safe_text_characters` 字段告知调用方，`tests/test_tts_safe_text_length.py`
  钉住"它必须存在、必须低于 `max_text_characters`、必须被 info 宣告"。
  
  **两个限制是两个问题**，这也是那个文件存在的理由：`protocol.MAX_TEXT_CHARACTERS`（400）
  答的是"请求会不会被拒"，`SAFE_TEXT_CHARACTERS`（60）答的是"听的人会不会听到断音"。
  两者之间那段区间里，服务照常接受、照常回 `status=PASS`，而音频在中途断掉——这正是把
  400 当成可用长度会踩的坑。安全值取 60 而非 80：80 也守住了，但只剩 +209 ms 余量。
  
  `MAX_TEXT_CHARACTERS` 一个字节没动——它是 `eidolon_sdk.biz.contracts.local_tts` 的跨仓库
